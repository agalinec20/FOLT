...
<head>
  <meta charset="UTF-8">
  <title>Provjera rada poslužitelja</title>
  <style>
...
<div id="container">
  ...
  <div id="main">
    <h2>Provjera rada poslužitelja</h2>
    <%
      Integer status = (Integer) request.getAttribute("status");
      if (status != null) {
        if (status == 200) {
    %>
      <p style="color:green;">Poslužitelj je dostupan (HTTP 200 OK)</p>
    <%
        } else {
    %>
      <p style="color:red;">Poslužitelj nije dostupan! Status: <%= status %></p>
    <%
        }
      } else {
    %>
      <p style="color:gray;">Status nije dohvaćen.</p>
    <%
      }
    %>
  </div>
