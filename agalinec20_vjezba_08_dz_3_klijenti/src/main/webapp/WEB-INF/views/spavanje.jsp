...
<head>
  <meta charset="UTF-8">
  <title>Spavanje poslužitelja</title>
  <style>
    body {
...
<div id="container">
  ...
  <div id="main">
    <h2>Spavanje poslužitelja</h2>

    <form method="post" action="${pageContext.servletContext.contextPath}/mvc/tvrtka/admin/spava">
      <p>Unesite trajanje spavanja (u sekundama):</p>
      <input type="number" name="vrijeme" min="1" required />
      <input type="submit" value="Aktiviraj spavanje" />
    </form>

    <%
      Integer status = (Integer) request.getAttribute("status");
      Integer vrijeme = (Integer) request.getAttribute("vrijeme");
      if (status != null) {
        if (status == 200) {
    %>
          <p style="color:green;">Poslužitelj je stavljen u stanje spavanja na <%= vrijeme %> sekundi.</p>
    <%
        } else {
    %>
