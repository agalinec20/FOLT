...
<head>
  <meta charset="UTF-8">
  <title>Status poslužitelja</title>
  <style>
    body {
...
<div id="container">
  ...
  <div id="main">
    <h2>Status poslužitelja</h2>
    <ul>
      <%
        if (request.getAttribute("status") != null) {
      %>
      <li>Status operacije: <strong><%= request.getAttribute("status") %></strong></li>
      <%
        }
        if (request.getAttribute("samoOperacija") != null && !((Boolean) request.getAttribute("samoOperacija"))) {
      %>
      <li>Status poslužitelja: <strong><%= request.getAttribute("statusT") %></strong></li>
      <li>Status poslužitelja za registraciju: <strong><%= request.getAttribute("statusT1") %></strong></li>
      <li>Status poslužitelja za partnere: <strong><%= request.getAttribute("statusT2") %></strong></li>
      <%
        }
      %>
    </ul>
